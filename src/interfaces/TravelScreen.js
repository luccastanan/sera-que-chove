...
import {asyncForEach, differenceOfDatesInDays} from '../util';
...
export default class TravelScreen extends Component{
    ...
    _touchRegister = async () => {
        if (this.state.places.length == 0){
            ...
            await asyncForEach(this.state.places, async (e, i) => {
                if (!e.id) {
                    let differenceDays = differenceOfDatesInDays(e.date, new Date())
                    ...
                    await asyncForEach(e.restaurants, async (rest, restI) => {
                        if (rest.image) {
